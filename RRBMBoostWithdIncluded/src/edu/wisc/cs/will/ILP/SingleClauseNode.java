					...
					if (parent == null || !parent.posExampleAlreadyExcluded(posEx)) { // Don't look at THIS node or we'll have an infinite loop.
						...
						if (theILPtask.regressionTask && !theILPtask.oneClassTask) {
							...
							if (!theILPtask.mlnRegressionTask) { // Example counting for MLN tasks later
								...
								double output = ((RegressionExample) posEx).getOutputValue(); //added by Navdeep Kaur
								//Utils.println("Navdeep: neg example: "+posEx.toString()+" :output: "+output);    // added by Navdeep Kaur
...
//		}
	...
	public double regressionFitForRBMs() throws SearchInterrupted {
		
		LearnOneClause  theILPtask = (LearnOneClause) task;
			
		RegressionInfoHolderForRBM holder = (RegressionInfoHolderForRBM) getRegressionInfoHolder();
		...
		double result = 0;
		if (holder.getTrueStats() != null) { result += holder.getRBMCost(holder.getTrueStats()); } else { Utils.waitHere("Why is true stats null?" + this.literalAdded); }
		if (holder.getFalseStats() != null) { result += holder.getRBMCost(holder.getFalseStats()); } else { Utils.waitHere("Why is false stats null?"+ this.literalAdded); }
...
//	}
	...
	*/
	...
	public Map<String,Double> RBMVectorIfTrue() {
		
		Map<String,Double> leafValues = new HashMap<String,Double>();
		RegressionInfoHolder holder = getRegressionInfoHolder();
		leafValues.put("d" ,holder.getTrueStats().getdforRBM());
		leafValues.put("c" ,holder.getTrueStats().getCforRBM());
		leafValues.put("U0",holder.getTrueStats().getU0forRBM());
		leafValues.put("U1",holder.getTrueStats().getU1forRBM());
		leafValues.put("W" ,holder.getTrueStats().getWforRBM());
		
		return leafValues;
	}
	
	public Map<String,Double> RBMVectorIfFalse() {
		
		Map<String,Double> leafValues = new HashMap<String,Double>();
		RegressionInfoHolder holder = getRegressionInfoHolder();
		leafValues.put("d" ,holder.getFalseStats().getdforRBM());
		leafValues.put("c" ,holder.getFalseStats().getCforRBM());
		leafValues.put("U0",holder.getFalseStats().getU0forRBM());
		leafValues.put("U1",holder.getFalseStats().getU1forRBM());
		leafValues.put("W" ,holder.getFalseStats().getWforRBM());
	...
	public String reportRegressionRuleAsStringForRBM(boolean examplesFlipFlopped) throws SearchInterrupted {
		String result = "FOR " + getClauseHead() + " ";
		
		List<Literal> bodyLits = getClauseBody();
		if (Utils.getSizeSafely(bodyLits) < 1) { result += "output = " + Utils.toString((examplesFlipFlopped ? RBMVectorIfFalse() : RBMVectorIfTrue()), ",  "); }
		else {
			boolean firstTime = true;
			result += "IF (";
			for (Literal lit : bodyLits) {
			...
			result += ") THEN output = " + Utils.toString((examplesFlipFlopped ? RBMVectorIfFalse() : RBMVectorIfTrue()), ",  ") + " ELSE output = " + Utils.toString((examplesFlipFlopped ? RBMVectorIfFalse() : RBMVectorIfTrue()), ",  ");
		}
		return result + ";" + (extraString == null ? "" : " // " + extraString);
	}
