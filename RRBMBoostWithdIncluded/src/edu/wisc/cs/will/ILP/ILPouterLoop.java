...
public class ILPouterLoop implements GleanerFileNameProvider {
    ...
    private boolean        learnRBM                            = false;   // Added by Navdeep Kaur
    ///////////////////////////////////////////////////////////////////
                ...
                if (!foundUncoveredPosSeed) { // Since there is a minimum coverage on the best clause, some seeds won't be covered and we might run out of seeds before meeting some other termination criterion.
                    ...
                    if (bestNode != null && bestNode != savedBestNode) { // Also need to check to make sure we didn't simply return the previous root when doing tree-structured learning.
                        ...
                        if (learningTreeStructuredTheory) {
                            ...
                            Map<String,Double> RBMParametersTrue = null; // Added By Navdeep Kaur
                            ...
                            {
                            	RBMParametersTrue = bestNode.RBMVectorIfTrue();
                            }
                            else {
                            	if (!learnOCCTree) {
                            ...
                            if (atMaxDepth || goodEnoughFitTrueBranch ||
                                ...
                                } else if(learnRBM) {                                             // Added By Navdeep Kaur
                                	leaf = createLeafNodeFromCurrentExamples(RBMParametersTrue);
                                }
                                 else {
                                ...
                                if (learnMultiValPredicates) {
                                	...
                                	newTreeNode.setRegressionMapIfLeaf(RBMParametersTrue);
                                }
                                else {
                            ...
                            Map<String,Double> RBMParametersFalse = null; // Added By Navdeep Kaur
                            
                            if (learnMLNTheory) {
                            	meanFalse = bestNode.mlnRegressionForFalse();
                            } else if(learnRBM)                             // Added By Navdeep Kaur
                            {
                            	RBMParametersFalse = bestNode.RBMVectorIfFalse();
                            }else {
                            	if (!learnOCCTree) {
                            ...
                            //	newClause.getLength()   >  maxTreeDepthInLiterals  ||
                                ...
                                if (learnMultiValPredicates) {
                                	...
                                	leaf = createLeafNodeFromCurrentExamples(RBMParametersFalse);
                                } else {
                                ...
                                if (learnMultiValPredicates) {
                                	...
                                	newTreeNode.setRegressionMapIfLeaf(RBMParametersFalse);
                                } else {
                          ...
                          //      		(parentOfCurrentNode == null ? "null" :parentOfCurrentNode.getFullNodeTest()));
                            ...
                            if (learnMLNTheory && !learningTreeStructuredTheory) {
                            	...
                            	for (Example eg : coveredPosExamplesThisCycle) {
									((RegressionRDNExample)eg).setOutputValue(((RegressionRDNExample)eg).getOutputValue() - reg); 
								}
                            }
                        }
		...
		//if (learnMLNTheory) {
			if (currentTask.getNode() instanceof TreeStructuredTheoryInteriorNode) {
				if (isLearnMultiValPredicates()) {
				   ...
				   val = createLeafNodeFromCurrentExamples(((TreeStructuredTheoryInteriorNode)currentTask.getNode()).getRegressionMapIfLeaf());
				   variance = createRBMLeafNodeFromCurrentExamples(currentTask.getPosExamples(), ((TreeStructuredTheoryInteriorNode)currentTask.getNode()).getRegressionMapIfLeaf());
				}
				else {
...
//			val =  createLeafNodeFromCurrentExamples(currentTask.getPosExamples());
		...
		TreeStructuredTheoryLeaf leaf = new TreeStructuredTheoryLeaf(currentTask.getNode().getWeightedCountOfPositiveExamples(),
																	 ...
																	 (isLearnRBM() ? variance :	 computeVarianceOverTheseExamples(currentTask.getPosExamples())),
																	 val,
   ...
   private Term createLeafNodeFromCurrentExamples(Map<String,Double> value) {
	   List<Term> terms = new ArrayList<Term>();
	   	  	  
	   if(value!=null)
	   {
	   		for (String key : value.keySet()) {
				   double val = value.get(key);
				   terms.add(innerLoopTask.stringHandler.getFunction(key,innerLoopTask.stringHandler.getNumericConstant(val)));   
		   }
	   }
	   else {
		   terms.add(innerLoopTask.stringHandler.getFunction("d",innerLoopTask.stringHandler.getNumericConstant(0.0)));
		   terms.add(innerLoopTask.stringHandler.getFunction("c",innerLoopTask.stringHandler.getNumericConstant(0.0)));   
		   terms.add(innerLoopTask.stringHandler.getFunction("W",innerLoopTask.stringHandler.getNumericConstant(0.0)));
		   terms.add(innerLoopTask.stringHandler.getFunction("U0",innerLoopTask.stringHandler.getNumericConstant(0.0)));
		   terms.add(innerLoopTask.stringHandler.getFunction("U1",innerLoopTask.stringHandler.getNumericConstant(0.0)));
	   }
   ...
   private double createRBMLeafNodeFromCurrentExamples(Collection<Example> currentExamples, Map<String,Double> rbmParameters)
   {
	    double result;
		...
		double phi =0.0;
		if(rbmParameters!=null)
		{
			phi  =rbmParameters.get("d") + (Math.log((1 + Math.exp(rbmParameters.get("c") + rbmParameters.get("U1") +rbmParameters.get("W")))/ (1 + Math.exp(rbmParameters.get("c") + rbmParameters.get("U0") + rbmParameters.get("W")))));
		}
		result = ((0.5*Math.pow(phi, 2)*weightedCount) + 0.5*totalSquaredOfOutput - phi*totalOfOutputValues)/ weightedCount;
   ...
   public void resetAll() {
      ...
      // innerLoopTask.stringHandler.reportStats();
        ...
        */
        ...
        if (learningTreeStructuredTheory) {
        	...
        	if(innerLoopTask.rbmRegressionTask)	{
        	 totalVariance = firstTask.getVarianceForRBM();
        	}
        	else {
        	 totalVariance = firstTask.getVariance();
        	}
    ...
    public  void setLearnRBM(boolean val) {
    	learnRBM = val;
    	innerLoopTask.setRBMRegressionTask(val);
    }
