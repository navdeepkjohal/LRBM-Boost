...
public class SavedClause implements Serializable {
	...
	public SavedClause(Gleaner caller, SingleClauseNode clause, long nodeCountWhenSaved, long acceptableNodeCountWhenSaved, boolean examplesFlipFlopped, String annotation, String clauseCreator) {
		...
		try {
			...
			if (((LearnOneClause) caller.getTaskBeingMonitored()).regressionTask && !((LearnOneClause) caller.getTaskBeingMonitored()).oneClassTask && !((LearnOneClause) caller.getTaskBeingMonitored()).rbmRegressionTask) {
				ruleAsString = "\n " + clause.reportRegressionRuleAsString(examplesFlipFlopped);
			} else if(((LearnOneClause) caller.getTaskBeingMonitored()).rbmRegressionTask) {
				ruleAsString = "\n " + clause.reportRegressionRuleAsStringForRBM(examplesFlipFlopped);
			}
			else {
