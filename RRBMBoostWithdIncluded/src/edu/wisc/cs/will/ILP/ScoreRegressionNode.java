...
public class ScoreRegressionNode extends ScoreSingleClauseByAccuracy {
	...
	private boolean forRBMs = false; 
		
	public ScoreRegressionNode() {
		this(false, false);
	}
	public ScoreRegressionNode(boolean useMLNs, boolean useRBMs) {
		super();
		forMLNs = useMLNs;
		forRBMs = useRBMs;
	}
	...
	//	node.computeCoverage(); // Do we need this?
		...
		if(forRBMs)
		{
			fit = node.regressionFitForRBMs();
		}
		else
		{
			fit = (forMLNs ? node.regressionFitForMLNs() : node.regressionFit());
		}
