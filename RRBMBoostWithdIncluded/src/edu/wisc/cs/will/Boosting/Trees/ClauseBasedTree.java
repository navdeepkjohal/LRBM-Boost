...
//		System.out.println("sum:: "+sum);
	...
	protected RegressionValueOrVector getRegressionClauseWt(Clause clause, Example ex) {
		...
		if (! ( y instanceof Variable)) {
			...
			RegressionValueOrVector value = null;
			if(setup.cmdArgs.isLearnRBM()){
				Map<String,Double> value2 = (BoostingUtils.getRegressionMapFromMap(y)).getRegressionMap();
				value = new RegressionValueOrVector(value2.get("d") + (Math.log((1 + Math.exp(value2.get("c") + value2.get("U1") + value2.get("W")))/ (1 + Math.exp(value2.get("c") + value2.get("U0") + value2.get("W"))) )) );
			}
			else {
				value = BoostingUtils.getRegressionValueOrVectorFromTerm(y);
			}
