...
public class BoostingUtils {
	...
	// Added by Navdeep Kaur for RRBM-Boost
	public static RegressionValueOrVector getRegressionMapFromMap(Term leafTerm) {
		if (leafTerm instanceof ConsCell) {
			ConsCell valarray = (ConsCell)leafTerm;
			Map<String,Double> regMap = new HashMap<String,Double>();  
			for (Term term : valarray) {
				if(term instanceof Function)
				{
					//System.out.println(((Function) term).functionName.toString());
					//System.out.println((((Function) term).getArgument(0)));
					regMap.put(((Function) term).functionName.toString(), ((NumericConstant)(((Function) term).getArgument(0))).value.doubleValue());
				}
			}
			
			return new RegressionValueOrVector(regMap);
		}
		Utils.error("Uknown type of Map in leaf: " + leafTerm.toPrettyString());
		return null;
	}
