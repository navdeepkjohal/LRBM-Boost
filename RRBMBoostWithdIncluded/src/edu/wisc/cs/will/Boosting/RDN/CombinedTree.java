	...
	public static void addToFinalSet(List <RegressionRDNExample> dataset){
		...
		if (CombinedTree.finalDataSet.size() == 0){
			...
			for(RegressionRDNExample example: dataset){
				...
				double prob = example.getProbOfExample().getProbOfBeingTrue();
				double phi = Math.log(prob/(1-prob));
				example.setOutputValue(phi);
				CombinedTree.finalDataSet.add(example);
				Utils.println("example:: "+example.toString()+" :: modelRegression :: "+phi+" :: "+example.getOutputValue());
			}
			System.out.println(CombinedTree.finalDataSet.size());
