...
import edu.wisc.cs.will.Utils.RegressionValueOrVector; // Added by Navdeep Kaur for -noBoost
/**
 * @author Tushar Khot
 *
 */
public class LearnBoostedRDN {
	...
	public void learnRDN(String pred, ConditionalModelPerPredicate rdn, SRLInference sampler, String yapFile, RunBoostedModels caller, int numMoreTrees) { // Thought we needed the 'caller' but we don't - leave it here, though, in case we do end up needing it.
		...
		List<RegressionRDNExample> newDataSet = buildDataSet(targetPredicate, sampler, getGradientFile(i)); //April 18
		CombinedTree.addToFinalSet(newDataSet); //April 18
		
	/*	if (cmdArgs.isCombineVal()) // Added by Navdeep to generate a deep combined Tree
		{
			ILPouterLoop outerLoop = setup.getOuterLooper();
			outerLoop.setMaxTreeDepth(10);
			outerLoop.setMaxTreeDepthInLiterals(12);
			//outerLoop.setMaxTreeDepthInLiterals(Math.max(4, (outerLoop.getMaxTreeDepth() + 1) * (outerLoop.innerLoopTask.maxFreeBridgersInBody + outerLoop.getMaxNumberOfLiteralsAtAnInteriorNode()))); 
			outerLoop.maxNumberOfClauses = 20;
			outerLoop.maxNumberOfCycles = 20;
			outerLoop.setMaxAcceptableNodeScoreToStop(0.0001);
		}	*/	
